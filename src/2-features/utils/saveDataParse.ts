...
// сбор всех ID сущьностей из tempData в indexedDB
...
// ищит ноду по заданному id и возвращает ее
function getNodeById(
  rootNode: IDataSave | TchildrenType | TNoteBody | undefined,
  find_id: string
) {
  const finder = (
    node: TchildrenType | TNoteBody
  ): TchildrenType | TNoteBody | null => {
    if (node.id === find_id) {
      return node;
    }
